...
switch method
    case 'xte'
        XTE = cross_track_error(public_vars.path, read_only_vars.mocap_pose(1:2));
        [w, public_vars] = my_pid(XTE, public_vars);
        v = read_only_vars.agent_drive.max_vel * max(0.3, 1 - k_v * abs(XTE));
        [vr, vl] = kinematics(v, -w, read_only_vars);

    case 'pure_pursuit'
        target = get_target(public_vars.path, read_only_vars.mocap_pose(1:2), L);
        [v, w] = pure_pursuit(target, read_only_vars.mocap_pose, read_only_vars, L);
        [vr, vl] = kinematics(v, w, read_only_vars);
